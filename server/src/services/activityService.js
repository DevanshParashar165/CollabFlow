...
export const createActivity = async ({ workspaceId, projectId = null, taskId = null, actorId, action, entityType, entityId, metadata = {} }) => {
  return Activity.create({ workspaceId, projectId, taskId, actorId, action, entityType, entityId, metadata });
};
...
const paginated = async (filter, page, limit) => {
  ...
  const [activities, total] = await Promise.all([
    Activity.find(filter).populate('actorId', 'name avatar').sort({ createdAt: -1 }).skip((options.page - 1) * options.limit).limit(options.limit),
    Activity.countDocuments(filter),
  ]);
  return { activities, pagination: { page: options.page, limit: options.limit, total, pages: Math.ceil(total / options.limit) } };
};
...
export const getProjectActivity = async (workspaceId, projectId, page, limit) => {
...
export default { createActivity, getWorkspaceActivity, getProjectActivity, getTaskActivity };